...
export function generateSKU(prefix: string, id: string): string {
  return `${prefix.toUpperCase()}-${id.slice(-6).toUpperCase()}`;
}
...
export function getStarDistribution(reviews: { rating: number }[]): Record<number, number> {
...
export function getEstimatedDeliveryDate(days: number): string {
  const date = new Date();
